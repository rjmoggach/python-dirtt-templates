...
#        sys.exit(-6)
    ...
    print "\nEnter the sequence_name:\n\tEg. vlo"
    sequence_name=raw_input("\tsequence_name >  ")
    sequence_path=os.path.join(project_path_full, "work", sequence_name)
    ...
    if not os.path.isdir(sequence_path):
        ...
        c.run()
        print "\n\tCreated Sequence Tree. [ %s ]" % sequence_name
    else:
        print "\n\tSequence Exists. [ %s ]" % sequence_name

    print "\nEnter a shot_name or comma-separated list(NO SPACES):\n\tEg. vst010"
    shot_name_raw=raw_input("\tshot_names >  ")
    ...
    for shot_name in shot_list:
        ...
        if not os.path.isdir(shot_path):
            ...
            c.run()
            print "\nCreated Shot Tree. [ %s ]" % shot_name
        else:
            print "\nShot Exists. [ %s ]" % shot_name
